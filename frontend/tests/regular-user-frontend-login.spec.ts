import { test, expect } from '@playwright/test';

test('regular user login through frontend interface', async ({ page }) => {
  const frontendBase = 'https://local.lchaty.com:5173';
  
  // Navigate to the login page
  await page.goto(frontendBase);
  
  // Find and fill the login form
  try {
    await page.waitForSelector('input', { timeout: 5000 });
    
    // Look for username/email field
    const usernameField = page.locator('input[type="text"], input[type="email"], input[name*="user"], input[placeholder*="user"], input[placeholder*="User"], input[placeholder*="name"]').first();
    await usernameField.fill('lesterTester');
    
    // Look for password field
    const passwordField = page.locator('input[type="password"]').first();
    await passwordField.fill('HardPass1234');
    
    // Look for submit button
    const submitButton = page.locator('button[type="submit"], button:has-text("Login"), button:has-text("Sign In"), input[type="submit"]').first();
    await submitButton.click();
    
    // Wait for navigation or response
    await page.waitForTimeout(3000);
    
    console.log('After login attempt, current URL:', page.url());
    
    // Check if we successfully logged in
    if (page.url() !== frontendBase) {
      console.log('✅ Successfully navigated after login');
    } else {
      console.log('❌ Still on login page');
    }
    
    // Try to verify we're authenticated by checking for user interface elements
    const appContent = await page.locator('#app, [data-testid="dashboard"], [data-testid="chat"], .dashboard, .chat').count();
    if (appContent > 0) {
      console.log('✅ Found app content, login appears successful');
      
      // Try to check if we can see user-specific content
      const userIndicator = await page.locator('[data-testid="user-menu"], .user-info, .username').count();
      if (userIndicator > 0) {
        console.log('✅ Found user indicator elements');
      }
    } else {
      console.log('❌ No app content found');
    }
    
    // Take a screenshot for debugging
    await page.screenshot({ path: 'regular-user-login-result.png', fullPage: true });
    console.log('Screenshot saved as regular-user-login-result.png');
    
  } catch (error) {
    console.error('Login test failed:', error);
    await page.screenshot({ path: 'regular-user-login-error.png', fullPage: true });
    throw error;
  }
});